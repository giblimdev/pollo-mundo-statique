...
export default function Page() {
  ...
  const [nbPoules, setNbPoules] = React.useState<number>(500);
  const [tauxPonte, setTauxPonte] = React.useState<number>(0.85);

  // Prix pilotés en COP (pas = 10 COP)
  const [prixOeufCOP, setPrixOeufCOP] = React.useState<number>(500); // ~0,30€
  ...
  const profitMoisEUR = profitJourEUR * 30;
    ...
    <TooltipProvider>
      <div className="mx-auto w-full max-w-5xl space-y-6 p-4 md:p-6">
        ...
        <div className="grid gap-4 md:grid-cols-4">
          ...
          <Metric
            label="Profit / moi"
            value={formatBothFromEUR(profitMoisEUR)}
            hint={`Revenu ${formatBothFromEUR(
              revenuJourEUR
            )} − Aliment ${formatBothFromEUR(coutAlimentJourEUR)}`}
            tone={rentabiliteOK ? "good" : "bad"}
